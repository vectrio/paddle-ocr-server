...
if command -v tmux >/dev/null 2>&1; then
	if tmux has-session -t "${SERVE_SESSION_NAME}" 2>/dev/null; then
		...
		tmux new-session -d -s "${SERVE_SESSION_NAME}" "cd \"${SERVE_WORKDIR}\" && source venv/bin/activate && paddlex --serve --pipeline \"${PIPELINE_CONFIG_PATH}\" --host \"${SERVE_HOST}\" --port \"${SERVE_PORT}\" --workers 4"
	...
	nohup bash -lc "cd \"${SERVE_WORKDIR}\" && source venv/bin/activate && paddlex --serve --pipeline \"${PIPELINE_CONFIG_PATH}\" --host \"${SERVE_HOST}\" --port \"${SERVE_PORT}\" --workers 4" > paddlex-serve.log 2>&1 &
